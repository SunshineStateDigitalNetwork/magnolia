...
def argument_parser():
    ...
    harvest_parser = subcommand_parsers.add_parser('harvest', help='magnolia harvest interactions',
                                                   parents=[config_parser])
    harvest_parser.add_argument('-r', '--run', action='store_true', help='run harvest for all config entries')

    # transform subcommand & args
    transformation_parser = subcommand_parsers.add_parser('transform', help='magnolia transformation interactions',
                                                          parents=[config_parser])
...
def interactive_run(magnolia_config, config_parser, subcommand, profile, *args, **kwargs):
    ...
    if subcommand == 'harvest':
        ...
        transform(magnolia_config, config_parser[config_parser.sections()[selection - 1]],
                  [k for k in config_parser.sections()][selection - 1], profile, verbosity=verbosity,
...
empty_magnolia_cfg = r'''# Please see magnolia configuration documentation: www.example.org"
...
empty_harvest_cfg = r'''# Please see magnolia configuration documentation: www.example.org"

[magnolia_provider_key]
oaiendpoint = 
setlist = 
metadataprefix =                     
'''

empty_scenario_cfg = r'''# Please see magnolia configuration documentation: www.example.org"
